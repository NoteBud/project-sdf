...
class Sorm
{
     ...
     * @return array|bool
     */
    public function read(array $conditions = []): array|bool
    {
        if (!$this->connection) {
            error_log("Connection not established");
            return [];
    ...
    public function update(array $data, array $conditions): bool
    {
        ...
        $setClauses = [];
        foreach ($data as $key => $value) {
            $setClauses[] = "`$key` = :$key"; // Use backticks
        }
        ...
        if (!empty($conditions)) {
            $conditionClauses = [];
            foreach ($conditions as $key => $value) {
                $conditionClauses[] = "`$key` = :cond_$key"; // Use backticks
            }
    ...
    public function join(
        ...
        $select = empty($select)
            ? "*"
            : implode(", ", array_map(fn($field) => "`$field`", $select)); // Use backticks
...
abstract class Model
{
    ...
    public static function tableName(): string
    {
        $class = static::class;
        $array = explode("\\", $class);
        return strtolower(end($array)) . "s"; // Assumes table name is pluralized class name
    }
...
class Migrator
{
    ...
    public function __construct(string $model, PDO $pdo)
    {
        ...
        if (!method_exists($model, "tableName")) {
            throw new \Exception(
                "Model $model must define a tableName() method."
            );
        }
    ...
    public function run(int $mode = 1): string|null
    {
        ...
        foreach ($this->fields as $name => $attributes) {
            ...
            if ($attributes["default"] !== null) {
                $default = is_string($attributes["default"])
                    ? "'{$attributes["default"]}'"
                    : $attributes["default"];
